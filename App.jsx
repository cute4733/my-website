...
export default function App() {
  ...
  const handleConfirmBooking = async () => {
    if(!bookingData.name || !bookingData.phone || !bookingData.date || !bookingData.time) return;
      ...
      <main className="pt-20">
        {bookingStep === 'form' ? (
          <div className="max-w-2xl mx-auto px-6 py-12">
            ...
            <div className="bg-white border border-[#EAE7E2] mb-6 p-6 shadow-sm flex flex-col sm:flex-row gap-6">
               <div className="w-full sm:w-32 h-40 sm:h-32 bg-gray-100 flex-shrink-0">
                  {selectedItem?.images?.[0] ? (
                    <img src={selectedItem.images[0]} className="w-full h-full object-cover" alt="selected" />
                  ) : <div className="w-full h-full flex items-center justify-center text-gray-300"><ImageIcon /></div>}
               </div>
               <div className="flex-1 flex flex-col justify-between">
                  <div className="space-y-1">
                    <p className="text-[10px] text-[#C29591] tracking-widest uppercase font-bold">預約款式</p>
                    <p className="text-sm font-medium">{selectedItem?.title} <span className="text-gray-400 font-light mx-2">/</span> {selectedAddon?.name || '無附加項目'}</p>
                  </div>
                  <div className="flex justify-between items-end border-t pt-4">
                    <div className="text-left">
                      <p className="text-[10px] text-gray-400 tracking-widest uppercase">總時長</p>
                      <p className="text-sm font-light">{(Number(selectedItem?.duration) || 0) + (Number(selectedAddon?.duration) || 0)} min</p>
                    </div>
            ...
            <div className="bg-white border border-[#EAE7E2] p-8 shadow-sm space-y-8">
              ...
              <div className="space-y-4">
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest flex items-center gap-2"><CalendarIcon size={14} /> 選擇日期</p>
          ...
          <div className="max-w-md mx-auto py-20 px-6">
            ...
            <div className="bg-white border border-[#EAE7E2] shadow-xl relative overflow-hidden">
              <div className="w-full h-48 bg-gray-100">
                 {selectedItem?.images?.[0] && <img src={selectedItem.images[0]} className="w-full h-full object-cover" alt="done" />}
              </div>
              <div className="p-8 space-y-6">
                <div className="border-b border-dashed pb-4">
                  <h3 className="text-lg font-medium text-[#463E3E]">{bookingData.name} 先生/小姐</h3>
                  <p className="text-xs text-gray-400 font-mono">{bookingData.phone}</p>
                </div>
                <div className="space-y-3">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-400">預約日期</span>
                    <span>{bookingData.date}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-400">預約時間</span>
                    <span className="font-bold text-[#C29591]">{bookingData.time}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-400">款式名稱</span>
                    <span>{selectedItem?.title}</span>
                  </div>
                </div>
                <div className="pt-4 border-t flex justify-between items-end">
                  <span className="text-2xl font-bold">NT$ {((Number(selectedItem?.price) || 0) + (Number(selectedAddon?.price) || 0)).toLocaleString()}</span>
                  <span className="text-[10px] text-gray-300 uppercase tracking-widest">截圖保存畫面</span>
                </div>
              </div>
            </div>
          ...
          <div className="min-h-[calc(100vh-80px)] flex flex-col items-center justify-center px-6 text-center">
            ...
            <div className="w-full max-w-xl mb-12 shadow-2xl border border-[#EAE7E2]">
            ...
            <h2 className="text-4xl font-extralight mb-12 tracking-[0.4em] text-[#463E3E]">Beyond Expectation</h2>
          ...
          <div className="max-w-7xl mx-auto px-6 py-12">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-16">
              {filteredItems.map(item => (
                <div key={item.id} className="group bg-white border border-[#F0EDEA] shadow-sm">
                  ...
                  <div className="p-8 text-center">
                    <span className="text-[10px] text-[#C29591] tracking-[0.4em] uppercase mb-2 block">{item.category}</span>
                    <h3 className="text-[#463E3E] font-medium text-lg mb-4">{item.title}</h3>
                    ...
                    <button onClick={() => { setSelectedItem(item); setBookingStep('form'); window.scrollTo(0,0); }} className="bg-[#463E3E] text-white px-8 py-3.5 rounded-full text-xs tracking-[0.2em] w-full hover:bg-[#C29591] transition-colors">點此預約</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-[200] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-5xl max-h-[90vh] overflow-y-auto p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-10 border-b pb-4">
              <h3 className="tracking-[0.3em] font-light uppercase text-[#463E3E]">系統後台管理</h3>
              <button onClick={() => setIsBookingManagerOpen(false)}><X size={24}/></button>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
              <div className="lg:col-span-2 space-y-4">
                <h4 className="text-xs font-bold border-l-4 border-[#C29591] pl-2 tracking-widest uppercase">最新預約</h4>
                <div className="space-y-3">
                  {allBookings.map(b => (
                    <div key={b.id} className="border p-4 bg-[#FAF9F6] text-xs flex justify-between items-center">
                      <div className="flex gap-4 items-center">
                        <div className="w-12 h-12 bg-gray-200 overflow-hidden">
                          {b.itemImage && <img src={b.itemImage} className="w-full h-full object-cover" />}
                        </div>
                        <div>
              ...
              <div className="space-y-8 border-l lg:pl-10">
                <h4 className="text-xs font-bold border-l-4 border-[#C29591] pl-2 tracking-widest uppercase">公休設定</h4>
