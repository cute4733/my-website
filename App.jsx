    ...
    switch(tab) {
      ...
      case 'contact': return (
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <div className="bg-white p-10 border shadow-sm"><p className="text-xs text-gray-500 mb-6">如有任何疑問，歡迎加入 LINE 官方帳號諮詢<br/>(預約請直接使用網站功能)</p><a href="https://lin.ee/RNTAv2L" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-[#06C755] text-white px-8 py-3 rounded-full font-bold text-sm"><MessageCircle size={20} /> 加入 LINE 好友</a></div>
        </div>
      );
      default: return null;
    }
      ...
      <nav className="fixed top-0 w-full bg-white/90 backdrop-blur-md z-[500] border-b border-[#EAE7E2]">
        <div className="max-w-7xl mx-auto px-6 py-4 md:py-0 md:h-20 flex flex-col md:flex-row items-center justify-between">
          ...
          <div className="flex gap-3 md:gap-6 text-xs md:text-sm tracking-widest font-medium uppercase items-center overflow-x-auto no-scrollbar justify-center">
            ...
            {/* 修正 4: 設定圖示移除橫線 */}
            {isLoggedIn ? <div className="flex gap-4 border-l pl-4 flex-shrink-0"><button onClick={() => handleOpenUpload()} className="text-[#C29591] hover:scale-110"><Plus size={18}/></button><button onClick={() => setStatus(p=>({...p, mgrOpen:true}))} className="text-[#C29591] hover:scale-110 border-none outline-none no-underline"><Settings size={18}/></button></div> : <button onClick={() => setStatus(p=>({...p, adminOpen:true}))} className="text-gray-300 hover:text-[#C29591] flex-shrink-0"><Lock size={14}/></button>}
          </div>
        </div>
      </nav>
