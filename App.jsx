...
const CustomCalendar = ({ selectedDate, onDateSelect, specificHolidays }) => {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    days.push(
      <button
        ...
        className={`h-10 w-10 text-xs rounded-full flex items-center justify-center transition-all
          ${isDisabled ? 'text-gray-200 cursor-not-allowed line-through' : 'hover:bg-[#C29591] hover:text-white'}
          ${isSelected ? 'bg-[#463E3E] text-white' : 'text-[#463E3E]'}
  ...
  return (
    <div className="border border-[#EAE7E2] p-4 bg-white">
      <div className="flex justify-between items-center mb-4">
        <h4 className="text-sm font-medium tracking-widest">{currentYear}年 {currentMonth + 1}月</h4>
      ...
      <div className="mt-4 flex gap-4 text-[9px] text-gray-400 uppercase tracking-tighter">
        <div className="flex items-center gap-1"><div className="w-2 h-2 bg-[#463E3E] rounded-full"></div> 已選日期</div>
        <div className="flex items-center gap-1"><div className="w-2 h-2 bg-gray-200 rounded-full"></div> 店休/過去日期</div>
      </div>
    </div>
  );
};
      ...
      <main className="pt-20">
        {bookingStep === 'form' ? (
          <div className="max-w-2xl mx-auto px-6 py-12">
            ...
            <div className="bg-white border border-[#EAE7E2] p-8 shadow-sm space-y-10">
              ...
              {bookingData.date && (
                <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest flex items-center gap-2">
                    <Clock size={14}/> 2. 選擇預約時段 ({bookingData.date})
                  </p>
                  <div className="grid grid-cols-4 md:grid-cols-6 gap-2">
                    {TIME_SLOTS.map(t => {
                      const full = isTimeSlotFull(bookingData.date, t);
                      return (
                        <button key={t} disabled={full} onClick={() => setBookingData({...bookingData, time:t})} className={`py-2 text-[10px] border transition-all ${full ? 'bg-gray-100 text-gray-300 cursor-not-allowed' : bookingData.time===t ? 'bg-[#463E3E] text-white border-[#463E3E]' : 'bg-white text-gray-400 hover:border-[#C29591]'}`}>
                          {t}
                        </button>
                      );
                    })}
              ...
              <div className="pt-6 border-t">
                ...
                <button onClick={() => setBookingStep('none')} className="w-full text-center text-[10px] text-gray-400 uppercase tracking-widest mt-4">返回重新選擇</button>
              </div>
            </div>
          </div>
        ) : bookingStep === 'success' ? (
          <div className="max-w-md mx-auto py-20 px-6 animate-in fade-in zoom-in duration-500">
            ...
            <div className="bg-white border border-[#EAE7E2] shadow-xl p-8 space-y-6 relative overflow-hidden">
              ...
              <div className="border-b border-dashed pb-4 text-sm">
                <p className="text-[10px] text-gray-400 uppercase tracking-widest mb-1">Details</p>
                <div className="font-medium">{bookingData.name} 先生/小姐</div>
                <div className="text-[#463E3E] font-bold mt-2">{bookingData.date} {bookingData.time}</div>
                <div className="text-gray-400 text-xs mt-1">{selectedItem?.title}</div>
              </div>
              <div className="text-right">
                <span className="text-[10px] text-[#C29591] block uppercase font-bold">Amount Due</span>
                  ...
                  {allBookings.map(b => (
                    <div key={b.id} className="border p-4 bg-[#FAF9F6] text-xs flex justify-between items-center">
                      <div className="space-y-1">
                        ...
                        <div className="text-gray-400">{b.phone} | NT${b.totalAmount}</div>
                      </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-[120] flex items-center justify-center p-4">
          <div className="bg-white p-8 max-w-md w-full shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-8 border-b pb-4">
              <h3 className="tracking-widest font-light">款式管理</h3>
            ...
            <form onSubmit={handleItemSubmit} className="space-y-6">
              ...
              <input type="number" required className="w-full border-b py-2 outline-none" value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} placeholder="價格" />
